...
--模块复位单片机引脚
local RST_SCMWD_PIN = nil
--模块和单片机相互喂狗引脚
local WATCHDOG_PIN = nil
...
local function getscm(tag)
	...
	if get_scm_cnt > 0 then
		...
		if tag=="test" then
			--如果检测到高电平
			if pins.get(WATCHDOG_PIN) == 1 then				
				testcnt = testcnt+1
	...
	--检测结束
		...
		if tag=="normal" and not scm_active then
			--复位单片机
			pins.set(false,RST_SCMWD_PIN)
			sys.timer_start(pins.set,100,true,RST_SCMWD_PIN)
			print("wdt reset 153b")
			scm_active = true
		end
	end
	--如果检测到低电平，则表示单片机运行正常
	if pins.get(WATCHDOG_PIN) == 0 and not scm_active then
		scm_active = true
...
local function feedend(tag)
	...
	if tag=="normal" and testing then return end
	--相互喂狗引脚配置为输入
	pins.close(WATCHDOG_PIN)
	pins.setdir(pio.INPUT,WATCHDOG_PIN)
...
function feed(tag)
	...
	if scm_active or tag=="test" then
		...
		pins.set(false,RST_SCMWD_PIN)
		sys.timer_start(pins.set,1000,true,RST_SCMWD_PIN)
		print("wdt reset 153b",tag)
	end

	--相互喂狗引脚配置为输出，"模块开始对单片机喂狗"，输出2秒的低电平
	pins.close(WATCHDOG_PIN)
	pins.setdir(pio.OUTPUT,WATCHDOG_PIN)
	pins.set(false,WATCHDOG_PIN)
	print("wdt feed",tag)
	--2分钟启动下次正常喂狗
...
参数  ：
  rst_pin: 模块复位单片机引脚
  wdt_pin: 模块和单片机相互喂狗引脚
返回值：无
]]
function open(rst_pin, wdt_pin)
  if rst_pin and wdt_pin then
    RST_SCMWD_PIN = rst_pin
    WATCHDOG_PIN = wdt_pin
  else
    print("Don't run watchdog,invalid pin!")
    return
  end
  
  --模块复位单片机引脚，默认输出高电平
  pins.setdir(pio.OUTPUT1,RST_SCMWD_PIN)
  pins.set(true,RST_SCMWD_PIN)
  
  pins.setdir(pio.OUTPUT,WATCHDOG_PIN)
  pins.set(true,WATCHDOG_PIN)
  feed("normal")
end
