...
--[[
两种休眠处理逻辑：
第一种：底层core内部，自动处理，例如tcp发送或者接收数据时，会自动唤醒，发送接收结束后，会自动休眠；这部分不用lua脚本控制；
第二种：lua脚本使用pm.sleep和pm.wake自行控制，例如，uart连接外围设备，uart接收数据前，要主动去pm.wake，这样才
                  能保证前面接收的数据不出错，当不需要通信时，调用pm.sleep；如果有lcd的项目，也是同样道理
                 如果不是故意控制的不休眠，一定要保证pm.wake("A")了，有地方去调用pm.sleep("A")。

功耗：
1.不休眠时功耗至少30mA左右;
2.休眠后，在飞行模式下不到1mA，而非飞行模式的功耗还没有数据(后续补充)；
3.实际还与网络状态相关。
]]
