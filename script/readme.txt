...
使用IDE工具烧写软件时，一定要选择这2部分脚本才能保证正常运行！！！


第1部分：
lib：“库脚本”，请注意：这个目录中的脚本是所有应用使用IDE工具下载时都需要包含得！！！


第2部分：
demo：以下所有项目的“应用脚本”，xiaoman_gps_tracker下的脚本在Air810的小蛮GPS定位器硬件上运行，其余目录下的lua脚本都可以在Air810开发板上运行
定位器中运行。
...
default：Air810的默认出厂软件demo项目，可以使用时间线APP查看模块的GPS位置或者基站位置
...
write_imei：写imei号到设备的demo项目

xiaoman_gps_tracker\whole_test：针对Air810的小蛮GPS定位器硬件写的一个完整的demo项目，支持硬件的各种功能，只能用于Air810的小蛮GPS定位器硬件，不能用于开发板，也不能配合时间线APP使用，注意修改sck.lua中的后台地址
